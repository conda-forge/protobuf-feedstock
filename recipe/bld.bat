...
python -c "import os, fileinput; print(''.join([line.replace(' SYSTEM_PYTHON_VERSION', ' str(' + os.getenv('PY_VER', '').replace('.', '') + ')') for line in fileinput.input('python/dist/dist.bzl', inplace=True)]))"
